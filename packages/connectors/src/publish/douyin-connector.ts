...
export class DouyinPublishConnector implements PublishConnector {
  ...
  async publishContent(input: PublishContentInput): Promise<PublishContentResult> {
    if (!this.config.accessToken && !this.config.cookie) {
      return { success: false, errorCode: 'NO_CREDENTIALS', errorMessage: 'Douyin requires access_token or cookie' };
    }

    if (this.config.mode === 'browser_assist') {
      return { success: false, errorCode: 'BROWSER_ASSIST_REQUIRED', errorMessage: 'Use browser-runner for browser_assist mode' };
    }
    ...
    try {
      ...
      return {
        ...
        errorMessage: String(data.description || dataNode?.description || 'Unknown Douyin API error'),
        rawResponse: data,
      };
    } catch (err) {
      return {
        ...
        errorMessage: err instanceof Error ? err.message : 'Network error calling Douyin API',
      };
    }
  }
  ...
  async checkStatus(input: CheckStatusInput): Promise<CheckStatusResult> {
    try {
      ...
      const statusMap: Record<number, 'published' | 'pending_review' | 'failed' | 'rejected'> = {
        1: 'pending_review', 2: 'published', 3: 'failed', 4: 'rejected',
      };
  ...
  async uploadMedia(input: UploadMediaInput): Promise<UploadMediaResult> {
    return {
      success: false,
      errorMessage: 'Media upload via API requires init + chunk upload flow; use browser_assist',
    };
  }
}
