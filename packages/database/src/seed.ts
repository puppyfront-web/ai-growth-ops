...
export const INITIAL_ADMIN_EMAIL = process.env.ADMIN_EMAIL || 'admin@ai-growth-ops.local';
...
export async function seedDatabase(db: DatabaseClient): Promise<void> {
  const existing = await db.user.findFirst({ where: { email: INITIAL_ADMIN_EMAIL } });
  if (existing) {
