-- Restores the missing pre-CRM schema history for fresh installations.
-- Ordered before customer_entity because that migration references organizations.
-- Existing databases require backup, schema comparison and explicit baselining;
-- do not guess tenant ownership or silently rewrite historical mock records.
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM "User") OR to_regclass('organizations') IS NOT NULL THEN
    RAISE EXCEPTION 'Existing database requires the controlled baseline procedure in docs/delivery/database-upgrade.md';
  END IF;
END $$;
...
CREATE TABLE "lead_source_configs" (
    ...
    "name" TEXT NOT NULL,
    "token" TEXT NOT NULL,
    "defaultLevel" TEXT NOT NULL DEFAULT 'B',
    "defaultPlatform" TEXT NOT NULL DEFAULT 'douyin',
...
-- CreateIndex
CREATE UNIQUE INDEX "lead_source_configs_token_key" ON "lead_source_configs"("token");
