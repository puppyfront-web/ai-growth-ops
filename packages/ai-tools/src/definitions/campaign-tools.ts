...
export const campaignTools: ToolDefinition[] = [
  {
    ...
    description:
      '创建内容运营活动。可设置主题、平台、发布频率等。活动激活后会自动生成内容并发布。',
    ...
    execute: async (params, ctx) => {
      return apiCall(ctx)('/api/campaigns', {
        ...
        body: {
          ...
          scheduleConfig:
            params.scheduleFrequency || params.scheduleDays
              ? {
  ...
  },
  {
    ...
    inputSchema: z.object({
      status: z
        .string()
        .optional()
        .describe('筛选状态：draft/active/paused/completed'),
      page: z.number().optional().default(1),
      limit: z.number().optional().default(10)
    }),
    execute: async ({ status, page, limit }, ctx) => {
      const query = new URLSearchParams();
      if (status) query.set('status', status);
      query.set('page', String(page));
      query.set('limit', String(limit));
      return apiCall(ctx)(`/api/campaigns?${query}`);
    }
  },
  {
    ...
    execute: async ({ campaignId }, ctx) => {
      return apiCall(ctx)(`/api/campaigns/${campaignId}`);
    }
  },
  {
    name: 'start_campaign',
    ...
    execute: async ({ campaignId }, ctx) => {
      return apiCall(ctx)(`/api/campaigns/${campaignId}/start`, {
        method: 'PATCH'
      });
    }
  },
  {
    ...
    execute: async ({ campaignId }, ctx) => {
      return apiCall(ctx)(`/api/campaigns/${campaignId}/pause`, {
        method: 'PATCH'
      });
    }
  },
  {
    name: 'trigger_campaign_run',
    description: '立即触发一次运营活动执行，不等待定时计划。',
    inputSchema: z.object({
      campaignId: z.string().describe('活动ID')
    }),
    execute: async ({ campaignId }, ctx) => {
      return apiCall(ctx)(`/api/campaigns/${campaignId}/run-now`, {
        method: 'POST'
      });
    }
  }
