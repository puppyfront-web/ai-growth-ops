...
export class DouyinProvider implements PlatformProvider {
  ...
  async validateCredentials(config: PlatformAuthConfig): Promise<HealthCheckResult> {
    ...
    if (!config.accessToken) {
      return { valid: false, platform: this.platform, error: '缺少 Access Token 或 Cookie' };
    }
    return this.validateAccessToken(config.accessToken);
  }

  private async validateAccessToken(accessToken: string): Promise<HealthCheckResult> {
    try {
      ...
      if (res.status === 401) {
        return { valid: false, platform: this.platform, error: 'Token 已过期或无效 (401)' };
      }
  ...
  private async validateCookie(cookie: string): Promise<HealthCheckResult> {
    try {
      const res = await fetch('https://creator.douyin.com/aweme/v1/creator/data/billboard/', {
        headers: {
          'Cookie': cookie,
      ...
      if (res.status === 401) {
        return { valid: false, platform: this.platform, error: 'Cookie 已过期 (401)' };
      }
      return { valid: false, platform: this.platform, error: `验证失败 (HTTP ${res.status})` };
    } catch (err) {
      return { valid: false, platform: this.platform, error: `网络错误: ${(err as Error).message}` };
