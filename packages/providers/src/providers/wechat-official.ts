...
export class WechatOfficialProvider implements PlatformProvider {
  ...
  async validateCredentials(config: PlatformAuthConfig): Promise<HealthCheckResult> {
    ...
    if (!config.appId || !config.appSecret) {
      return { valid: false, platform: this.platform, error: '缺少 AppID/AppSecret 或 Cookie' };
    }
    return this.validateOfficialApi(config.appId, config.appSecret);
  }

  private async validateOfficialApi(appId: string, appSecret: string): Promise<HealthCheckResult> {
    try {
