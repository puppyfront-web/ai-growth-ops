...
export function incrementCounter(name: string, tags?: Record<string, string>, value = 1): void {
  ...
  metrics.push({ name, value: current + value, timestamp: Date.now(), tags, type: 'counter' });
}
...
export function getMetricsSummary(): Record<string, { count: number; latest: number; sum: number }> {
  const summary: Record<string, { count: number; latest: number; sum: number }> = {};
