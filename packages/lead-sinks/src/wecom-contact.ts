...
export class WeComContactSink implements LeadSink {
  ...
  async sync(lead: LeadData, config: LeadSinkConfig): Promise<SinkResult> {
    ...
    if (!corpId || !secret) {
      return {
        ...
        errorMessage: 'WeCom corpId and secret are required'
      };
    }
    ...
    try {
      ...
      if (!token) {
        return {
          ...
          errorMessage: 'Failed to get WeCom access token'
        };
      }

      // Create external contact
      const resp = await fetch(
        'https://qyapi.weixin.qq.com/cgi-bin/externalcontact/add?access_token=' +
          token,
        {
          method: 'POST',
  ...
  async testConnection(
    ...
    try {
      const token = await this.getAccessToken(corpId, secret);
      if (token)
        return { success: true, message: 'WeCom connection successful' };
