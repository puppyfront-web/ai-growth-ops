  ...
  async sync(lead: LeadData, config: LeadSinkConfig): Promise<SinkResult> {
    ...
    if (!appToken || !tableId) {
      return {
        ...
        errorMessage: 'Feishu appToken and tableId are required'
      };
    }
    ...
    try {
      ...
      if (!token) {
        return {
          ...
          errorMessage: 'Failed to get Feishu tenant token'
        };
      }
  ...
  async testConnection(
    ...
    try {
      ...
      if (!token)
        return {
          success: false,
          message: 'Failed to authenticate with Feishu'
        };
      ...
      if (data.code === 0) {
        return {
          success: true,
          message: 'Feishu Bitable connection successful'
        };
      }
      return {
        success: false,
        message: String(data.msg || 'Connection failed')
      };
    } catch (err) {
      return {
        success: false,
