...
export function createSchemaValidator<T>(inputSchema: object, outputSchema: object) {
  return {
    validateInput(data: unknown): T {
      const result = validateJsonSchema(data, inputSchema);
      if (!result.valid) throw new Error(`Input validation failed: ${result.errors?.join(', ')}`);
      return data as T;
    },
