...
export async function handleCampaignExecute(job: Job): Promise<void> {
  ...
  if (!run || !run.campaign)
    throw new Error(`CampaignRun ${campaignRunId} not found`);
  ...
  try {
    ...
    const title =
      contentResult.status === 'success' && contentResult.output
        ? ((contentResult.output as Record<string, unknown>).title as string) ||
    ...
    for (const platform of platforms) {
      try {
        ...
        const variantData =
          rewriteResult.status === 'success' && rewriteResult.output
        ...
        await db.contentVariant.create({
          data: {
            ...
            platform: platform as
              ...
              | 'baijiahao'
              | 'zhihu',
            contentType: campaign.contentType as
              | 'text_image'
              | 'video'
              | 'article',
        ...
        console.error(
          `[campaign] Variant creation failed for ${platform}:`,
          err
        );
      }
    }
    ...
    if (campaign.autoCompliance) {
      ...
      for (const variant of variants) {
        try {
          ...
          const passed =
            complianceResult.status === 'success' && complianceResult.output
              ? (complianceResult.output as Record<string, unknown>).passed !==
                false
              : true;
          ...
          console.error(
            `[campaign] Compliance failed for variant ${variant.id}:`,
            err
          );
        }
      }
    ...
    if (campaign.autoPublish) {
      ...
      for (const variant of approvedVariants) {
        ...
        if (!account) {
          console.log(
            `[campaign] No account for ${variant.platform}, skipping`
          );
          continue;
        }
