...
export async function handleResearchRun(job: Job<ResearchRunInput>): Promise<void> {
  ...
  try {
    ...
    for (const keyword of keywords.length > 0 ? keywords : ['热门内容']) {
      try {
        const resp = await fetchWithTimeout(`${RUNNER_URL}/assist/search-and-fetch-comments`, {
          method: 'POST',
          headers: {
        ...
        for (const video of results) {
          allPosts.push({
            ...
            commentCount: (video.comments as Array<Record<string, unknown>>)?.length ?? 0,
            metadata: { keyword },
          });
        ...
        job.log(`Search error for keyword "${keyword}": ${err instanceof Error ? err.message : String(err)}`);
        continue;
      }
    }
    ...
    try {
      ...
      if (result.status === 'success' && result.output) {
        ...
        const insights = [
          { type: 'pain_points', title: '用户痛点分析', data: output.painPoints as unknown },
          { type: 'popular_topics', title: '热门话题趋势', data: output.popularTopics as unknown },
          { type: 'content_angles', title: '内容创作角度', data: output.contentAngles as unknown },
        ];

        for (const insight of insights) {
          const items = Array.isArray(insight.data) ? insight.data : [insight.data];
      ...
      job.log(`AI insight generation failed: ${err instanceof Error ? err.message : String(err)}`);
      ...
      try {
        ...
        job.log(`Rule-based insight generation also failed: ${fallbackErr instanceof Error ? fallbackErr.message : String(fallbackErr)}`);
      }
    }
    ...
    job.log(`Research task ${researchTaskId} completed: ${allPosts.length} posts, ${allComments.length} comments`);
  } catch (err) {
    await db.researchTask.update({
      where: { id: researchTaskId },
      data: {
