...
export default function CompliancePage() {
  ...
  const { data: rules, isLoading } = useQuery({ queryKey: ['compliance-rules'], queryFn: getComplianceRules });
  ...
  const saveMutation = useMutation({
    mutationFn: () => updateComplianceRules({ sensitiveWords: localWords, forbiddenPhrases: localPhrases }),
  ...
  return (
    <div>
      <Breadcrumb />
      <PageHeader title="合规规则" description="配置敏感词、禁用话术和人工确认规则" />
      {rules && (
        <div className="max-w-2xl space-y-6">
          <div className="rounded-xl border bg-card p-5 space-y-4">
            ...
            <div className="flex gap-2">
              <input value={newWord} onChange={(e) => setNewWord(e.target.value)} className="flex-1 rounded-md border p-2 text-sm" placeholder="添加敏感词..." onKeyDown={(e) => {
                if (e.key === 'Enter' && newWord.trim()) { setLocalWords((w) => [...w, newWord.trim()]); setNewWord(''); }
              }} />
              <button onClick={() => { if (newWord.trim()) { setLocalWords((w) => [...w, newWord.trim()]); setNewWord(''); } }} className="rounded-md border px-4 py-2 text-sm hover:bg-accent">添加</button>
            </div>
          </div>
          <div className="rounded-xl border bg-card p-5 space-y-4">
            <h3 className="font-semibold">禁用话术</h3>
            {localPhrases.map((phrase) => (
              <div key={phrase} className="flex items-center justify-between rounded-lg border p-3">
                <span className="text-sm">{phrase}</span>
                <button onClick={() => setLocalPhrases((p) => p.filter((x) => x !== phrase))} className="text-xs text-destructive hover:underline">删除</button>
              </div>
            ))}
          </div>
          <div className="rounded-xl border bg-card p-5 space-y-4">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">{rules.autoReplyLimits.maxDailyReplies}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-sm">置信度阈值</span>
                <span className="text-sm font-medium">{rules.autoReplyLimits.confidenceThreshold}</span>
              </div>
            </div>
          </div>
          <div className="rounded-xl border bg-card p-5 space-y-4">
            ...
            <div className="space-y-3">
              {rules.humanConfirmRules.map((rule, i) => (
                <label key={i} className="flex items-center justify-between">
                  ...
                  <span className="text-xs text-muted-foreground">阈值 {rule.threshold}</span>
                </label>
              ))}
            </div>
          </div>
