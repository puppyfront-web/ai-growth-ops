  ...
  const columns: ColumnDef<ResearchTask>[] = [
    {
      ...
      cell: ({ getValue }) => {
        const labels: Record<string, string> = {
          ...
          comment_sampling: '评论采样'
        };
        return (
          <span>{labels[getValue() as string] ?? (getValue() as string)}</span>
        );
      }
    },
    {
        ...
        <StatusBadge
          ...
          label={
            researchStatusLabels[
              getValue() as keyof typeof researchStatusLabels
            ]
          }
        />
      )
    },
    {
      ...
      header: '操作',
      cell: ({ row }) => {
        const canRun = ['DRAFT', 'FAILED', 'PAUSED'].includes(
          row.original.status
        );
        const isPending =
        ...
        return (
          <div className="flex gap-2">
            <Link
              href={`/research/tasks/${row.original.id}`}
  ...
  return (
    <div>
      ...
      <DataTable
        ...
        onRetry={() => refetch()}
        emptyTitle="暂无调研任务"
        emptyDescription="创建一个调研任务来开始发现选题机会"
      />
    </div>
  );
}
