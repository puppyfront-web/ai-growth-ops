...
const actionColors: Record<string, string> = {
  ...
  publish: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300',
  sync: 'bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300',
  ai_generate: 'bg-purple-50 text-purple-700 dark:bg-purple-950 dark:text-purple-300',
    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        ...
        <div className="ml-auto">
          <ExportCSVButton url="/api/export/audit-logs.csv" filename="audit-logs.csv" label="导出日志" />
        </div>
      </div>
            ...
            return (
              <div key={log.id} className="rounded-lg border bg-card overflow-hidden">
                ...
                >
                  ...
                  <span className={`shrink-0 rounded px-2 py-0.5 text-xs font-medium ${actionColors[log.action] ?? 'bg-muted text-muted-foreground'}`}>
                    {log.action}
                  </span>
                  <div className="flex items-center gap-1.5 min-w-0 text-sm">
                    <FileText className="h-3.5 w-3.5 shrink-0 text-muted-foreground" />
                    <span>{log.entity ?? '-'}</span>
                    {log.entityId && <span className="text-muted-foreground truncate max-w-24">#{log.entityId.slice(0, 8)}</span>}
                  </div>
                  {log.user && (
                  ...
                  <span className="ml-auto shrink-0 text-xs text-muted-foreground">{formatDate(log.createdAt)}</span>
                </button>
                ...
                {isExpanded && hasChanges && (
                  <div className="border-t bg-muted/20 px-4 py-3">
                    <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
                      <div>
                        <p className="mb-1 text-xs font-medium text-muted-foreground">变更前</p>
                        <pre className="overflow-x-auto rounded bg-background p-2 text-xs">
                          {log.changes!.before != null ? JSON.stringify(log.changes!.before, null, 2) : '(无)'}
                        </pre>
                      </div>
                      <div>
                        <p className="mb-1 text-xs font-medium text-muted-foreground">变更后</p>
                        <pre className="overflow-x-auto rounded bg-background p-2 text-xs">
                          {log.changes!.after != null ? JSON.stringify(log.changes!.after, null, 2) : '(无)'}
                        </pre>
                      </div>
                    </div>
                  </div>
