...
export default function TeamPage() {
  ...
  const inviteMutation = useMutation({
    mutationFn: () =>
      inviteTeamMember({ email: inviteEmail, role: inviteRole }),
    onSuccess: () => {
  ...
  if (error)
    return <ErrorState message="加载团队成员失败" onRetry={() => refetch()} />;
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3 mb-6">
        ...
        <Card>
          <CardContent className="pt-6">
            ...
            <div className="text-sm text-muted-foreground">运营人员</div>
          </CardContent>
        </Card>
      </div>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <div className="relative">
                  <button
                    onClick={() =>
                      setRoleMenuId(roleMenuId === member.id ? null : member.id)
                    }
                    ...
                    <div className="absolute right-0 top-8 z-10 rounded-lg border bg-card shadow-lg p-1 min-w-[120px]">
                      {(['admin', 'operator', 'viewer'] as const).map((r) => (
                        <button
                          key={r}
                          onClick={() =>
                            roleMutation.mutate({ id: member.id, role: r })
                          }
