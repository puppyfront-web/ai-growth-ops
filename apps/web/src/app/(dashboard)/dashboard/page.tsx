...
export default function DashboardPage() {
  ...
  const { data: leadTrend } = useQuery({
    queryKey: [...queryKeys.dashboard.trends(dateRange), 'leads'],
    queryFn: getLeadTrend
  });
  const { data: platformTrend } = useQuery({
    queryKey: [...queryKeys.dashboard.trends(dateRange), 'platforms'],
  ...
  if (error)
    return (
      <ErrorState message="加载工作台数据失败" onRetry={() => refetch()} />
    );
  if (!data) return null;
