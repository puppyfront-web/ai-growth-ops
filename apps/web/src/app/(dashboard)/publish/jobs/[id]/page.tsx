...
function getTimelineStepState(
  ...
  const jobIndex = timelineStatuses.indexOf(
    jobStatus as (typeof timelineStatuses)[number]
  );
  if (jobIndex < 0) {
    if (jobStatus === 'NEED_MANUAL_REPAIR') {
      return stepIndex <= timelineStatuses.indexOf('RUNNING')
        ? 'done'
        : 'pending';
    }
    return 'pending';
...
export default function PublishJobDetailPage() {
  ...
  const { data: attempts } = useQuery({
    queryKey: ['publish-attempts', id],
    queryFn: () => getPublishAttempts(id)
  });
  ...
  if (isError) {
    const message =
      error instanceof ApiError ? error.message : '加载发布任务失败';
        ...
        <div className="rounded-xl border bg-card p-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div>
              ...
              <div>{publishModeLabels[job.mode]}</div>
            </div>
            <div>
              <span className="text-muted-foreground">类型</span>
              <div>{contentTypeLabels[job.contentType]}</div>
            </div>
            <div>
            ...
            <div>
              ...
              <div>{job.scheduledAt ? formatDate(job.scheduledAt) : '-'}</div>
            </div>
            <div>
          ...
          {job.externalUrl && (
            <div>
              <span className="text-sm text-muted-foreground">发布链接</span>
              <div>
                <a
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b">
                <th className="py-2 text-left text-sm text-muted-foreground">
                  序号
                </th>
                <th className="py-2 text-left text-sm text-muted-foreground">
                  状态
            ...
            <tbody>
              {attempts.map((a) => (
                <tr key={a.id} className="border-b last:border-0">
                  <td className="py-2 text-sm">#{a.attemptNo}</td>
                  <td className="py-2">
                    <StatusBadge status={a.status} />
                  </td>
                  <td className="py-2 text-sm text-destructive">
                    {a.error ?? '-'}
                  </td>
