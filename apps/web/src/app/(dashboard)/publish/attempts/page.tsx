...
import { publishStatusLabels, platformLabels } from '@/lib/constants';
import { formatDate } from '@/lib/utils';

export default function PublishAttemptsPage() {
  const { data: jobs, isLoading } = useQuery({ queryKey: ['publish-jobs-failed'], queryFn: () => listPublishJobs({ status: 'FAILED' }) });
          ...
          {jobs?.items?.map((job) => (
            <div key={job.id} className="rounded-xl border bg-card p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <StatusBadge status={job.status} label={publishStatusLabels[job.status]} />
                <span className="text-sm text-muted-foreground">重试 {job.retryCount} 次</span>
              </div>
