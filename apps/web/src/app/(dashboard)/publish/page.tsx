...
const columns: ColumnDef<PublishJob>[] = [
  ...
  },
  {
    ...
    header: '类型',
    cell: ({ getValue }) => (
      <span className="text-sm">{contentTypeLabels[getValue() as string]}</span>
    )
  },
  {
    accessorKey: 'mode',
    ...
    cell: ({ getValue }) => (
      <span className="text-xs rounded bg-muted px-1.5 py-0.5">
        {publishModeLabels[getValue() as string]}
      </span>
    )
  },
  {
    ...
    cell: ({ getValue }) => (
      <StatusBadge
        ...
        label={
          publishStatusLabels[getValue() as keyof typeof publishStatusLabels]
        }
      />
    )
  },
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className="flex gap-2">
        <Link
          href={`/publish/jobs/${row.original.id}`}
...
export default function PublishPage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({
    ...
    queryFn: () =>
      listPublishJobs(statusFilter ? { status: statusFilter } : undefined)
  });
