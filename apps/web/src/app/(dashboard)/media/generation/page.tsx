...
export default function MediaGenerationPage() {
  ...
  const { data: generatedAssets, isLoading: loadingHistory } = useQuery<
    MediaAsset[]
  >({
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {generationTypes.map((gt) => (
              ...
              >
                <gt.icon
                  className={`h-6 w-6 mb-2 ${genType === gt.key ? 'text-primary' : 'text-muted-foreground'}`}
                />
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      Provider:{' '}
                      {generateMutation.data.generationProvider ?? '-'}
                      {generateMutation.data.costEstimate != null &&
                        ` · 预估费用: $${generateMutation.data.costEstimate.toFixed(4)}`}
                    </div>
                    <div className="flex items-center gap-2 mt-2">
                      <StatusBadge
                        status={generateMutation.data.reviewStatus}
                        label={
                          generateMutation.data.reviewStatus ===
                          'pending_review'
                            ? '待审核'
                            : generateMutation.data.reviewStatus
                        }
