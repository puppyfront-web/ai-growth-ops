...
export default function PlatformsPage() {
  ...
  const [validateResult, setValidateResult] = useState<Record<
    string,
    unknown
  > | null>(null);
  ...
  const updateMutation = useMutation({
    mutationFn: (params: { id: string; [key: string]: unknown }) =>
      updatePlatformAccount(params.id, params),
    onSuccess: () => {
  ...
  const handleToggleCapability = (
    ...
    updateMutation.mutate({
      id: accountId,
      capabilities: [{ id: capId, enabled: !currentEnabled }]
    });
  };
  ...
  const handleSaveDialog = () => {
    ...
    if (dialog.mode === 'create') {
      ...
      if (
        dialog.authType === 'official_api' &&
        dialog.platform === 'wechat_official'
      ) {
  ...
  const openEditDialog = (account: PlatformAccount) => {
    const resolvedAuthType =
      account.authType ||
      (platformAuthOptions[account.platform] ?? [])[0] ||
      '';
    setDialog({
      mode: 'edit',
      ...
      cookie: '',
      appId:
        ((account.metadata as Record<string, unknown>)?.appId as string) ?? '',
      appSecret:
        ((account.metadata as Record<string, unknown>)?.appSecret as string) ??
        '',
      clientKey:
        ((account.metadata as Record<string, unknown>)?.clientKey as string) ??
        '',
      clientSecret:
  ...
  return (
    <div>
      ...
      <PageHeader
        title="平台账号"
        description="管理6大平台账号授权和能力"
        actions={
          <button
            onClick={() => setDialog(emptyDialog('create'))}
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
        {data?.map((account) => {
          ...
          const needsSetup = (account.metadata as Record<string, unknown>)
            ?.requiresManualSetup;

          return (
            <div
              key={account.id}
              className={`rounded-xl border bg-card p-5 flex flex-col ${needsSetup ? 'border-dashed opacity-70' : ''}`}
            >
              {/* Header */}
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">
                      {
                        platformLabels[
                          account.platform as keyof typeof platformLabels
                        ]
                      }
                    </p>
                  </div>
              ...
              <div className="flex items-center gap-2 mb-3">
                ...
                {account.authType && (
                  <span className="text-xs text-muted-foreground">
                    · {authTypeLabels[account.authType]}
                  </span>
                )}
              </div>
                  ...
                  {caps.map((cap) => (
                    ...
                    >
                      <span className="text-xs">
                        {capabilityLabels[cap.capabilityKey] ??
                          cap.capabilityKey}
                      </span>
                      <button
                        onClick={() =>
                      ...
                      >
                        <span
                          className={`pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow transition-transform ${cap.enabled ? 'translate-x-4' : 'translate-x-0'}`}
                        />
                      </button>
                    </div>
                  ))}
              ...
              {validateResult?.accountId === account.id && (
                <div
                  className={`mb-2 rounded-md p-2 text-xs ${validateResult.valid ? 'bg-green-50 dark:bg-green-950 text-green-700' : 'bg-red-50 dark:bg-red-950 text-red-700'}`}
                >
                  {validateResult.valid
                    ? '凭证有效'
                    : `无效: ${validateResult.error ?? '未知错误'}`}
                </div>
              )}
              ...
              <div className="flex items-center justify-between border-t pt-3 mt-auto">
                <div className="text-xs text-muted-foreground">
                  {typeof account.lastHealthCheckAt === 'string'
                    ? `检查: ${formatDate(account.lastHealthCheckAt)}`
                    : '未检查'}
                </div>
      ...
      {d && (
        ...
        >
          <div
            className="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-xl bg-background p-6 shadow-xl"
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className="text-lg font-semibold mb-4">
              {d.mode === 'create' ? '创建平台账号' : '编辑账号配置'}
            </h3>
            <div className="space-y-4">
                      ...
                      setDialog((prev) =>
                        prev
                          ? {
                              ...
                              name: `${platformLabels[p as keyof typeof platformLabels] ?? p} 账号`
                            }
                          : prev
                      );
                    }}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDialog((prev) =>
                        prev ? { ...prev, modeValue: e.target.value } : prev
                      )
                    }
                    className="mt-1 w-full rounded-md border p-2 text-sm"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setDialog((prev) =>
                        prev ? { ...prev, authType: e.target.value } : prev
                      )
                    }
                    className="mt-1 w-full rounded-md border p-2 text-sm"
                  >
              ...
              {d.authType === 'official_api' &&
                d.platform === 'wechat_official' && (
                  <div className="rounded-lg border bg-muted/30 p-3 space-y-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setDialog((prev) =>
                            prev ? { ...prev, appId: e.target.value } : prev
                          )
                        }
                        placeholder="wx..."
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setDialog((prev) =>
                            prev ? { ...prev, appSecret: e.target.value } : prev
                          )
                        }
                ...
                <div className="rounded-lg border bg-muted/30 p-3 space-y-3">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setDialog((prev) =>
                            prev ? { ...prev, clientKey: e.target.value } : prev
                          )
                        }
                        placeholder="aw..."
                        className="mt-1 w-full rounded-md border bg-background p-2 text-sm"
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="通过 OAuth 换取的 access_token"
                      className="mt-1 w-full rounded-md border bg-background p-2 text-sm"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setDialog((prev) =>
                          prev
                            ? { ...prev, refreshToken: e.target.value }
                            : prev
                        )
                      }
                      placeholder="refresh_token，用于自动续期"
                      className="mt-1 w-full rounded-md border bg-background p-2 text-sm"
                    />
                  </div>
                  <p className="text-xs text-muted-foreground">
                    在{' '}
                    <a
                      href="https://open.douyin.com/platform/doc"
                      target="_blank"
                      rel="noreferrer"
                      className="underline"
                    >
                    ...
                    → 我的应用 → 应用详情中获取 Client Key 和 Secret；Access
                    Token 通过 OAuth 授权码流程换取，需申请{' '}
                    <code className="bg-muted px-1 rounded">comment.list</code>
                    、
              ...
              {(d.authType === 'oauth' ||
                ...
                <div className="rounded-lg border bg-muted/30 p-3 space-y-3">
                  <h4 className="text-sm font-medium text-muted-foreground">
                    {d.authType === 'oauth' ? 'OAuth' : '官方 API'} 配置
                  </h4>
                  <div>
                    <label className="text-xs font-medium">Access Token</label>
                    <input
                ...
                <div className="rounded-lg border bg-muted/30 p-3 space-y-3">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      onChange={(e) =>
                        setDialog((prev) =>
                          prev ? { ...prev, cookie: e.target.value } : prev
                        )
                      }
                      rows={3}
                      placeholder="从浏览器中复制完整的 Cookie 值..."
                      className="mt-1 w-full rounded-md border bg-background p-2 text-sm"
                    />
                  </div>
                  <p className="text-xs text-muted-foreground">
                    在浏览器中登录平台后，打开开发者工具 → Network →
                    复制任意请求的 Cookie 头。
                  </p>
                </div>
              )}
