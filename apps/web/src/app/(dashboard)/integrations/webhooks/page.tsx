...
export default function WebhooksPage() {
  ...
  const createMutation = useMutation({
    mutationFn: () =>
      createWebhook({ url: webhookUrl, events: selectedEvents }),
    onSuccess: () => {
  ...
  if (error)
    return <ErrorState message="加载 Webhook 失败" onRetry={() => refetch()} />;
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            {webhookEvents.map((ev) => (
              <div key={ev.value} className="rounded-lg border p-3">
                ...
                <code className="mt-1 block rounded bg-muted px-1.5 py-0.5 text-[10px]">
                  {ev.value}
                </code>
              </div>
            ))}
          ...
          {webhooks.map((wh) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                <span
                  className={`rounded-full px-2 py-0.5 text-xs font-medium ${wh.status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900 dark:text-green-300' : 'bg-gray-100 text-gray-600 dark:bg-gray-800 dark:text-gray-400'}`}
                >
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {webhookEvents.map((ev) => (
                  <label
                    key={ev.value}
                    className="flex items-center gap-2 rounded-lg border p-2.5 cursor-pointer hover:bg-accent/50"
                  >
                    <input
                      type="checkbox"
