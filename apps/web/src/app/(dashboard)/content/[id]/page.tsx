...
export default function ContentDetailPage() {
  ...
  const initialTab =
    searchParams.get('tab') === 'variants' ? 'variants' : 'edit';
  const [activeTab, setActiveTab] = useState<'edit' | 'variants' | 'log'>(
    initialTab
  );
  ...
  useEffect(() => {
    ...
    setBody(item.body ?? '');
    const ids =
      ((item.metadata as Record<string, unknown>)?.mediaAssetIds as
        | string[]
        | undefined) ?? [];
  ...
  const { data: variantsData } = useQuery({
    queryKey: ['content-variants', id],
    queryFn: () => getContentVariants(id)
  });
  ...
  const existingPlatforms = useMemo(
    () => new Set(variants.map((v) => v.platform)),
    [variants]
  );
  const platformsToGenerate = allPlatforms.filter(
  ...
  const generateMutation = useMutation({
    mutationFn: (platforms: string[] | undefined) =>
      generatePlatformVariants(id, platforms),
    onSuccess: () => {
      setActiveTab('variants');
      qc.invalidateQueries({ queryKey: ['content-variants', id] });
    }
  });
  ...
  const updateVariantMutation = useMutation({
    ...
    }) => updateContentVariant(variantId, data),
    onSuccess: () => {
  ...
  const saveVariant = (variantId: string) => {
    ...
    updateVariantMutation.mutate({
      variantId,
      data: { title: editTitle, body: editBody, tags }
    });
  };
        ...
        <div className="space-y-3">
          ...
          {variants.map((v) => {
            ...
            return (
              <div
                key={v.id}
                className={`rounded-xl border bg-card p-4 flex gap-4 ${!canSelect && !isEditing ? 'opacity-60' : ''}`}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-2">
                    <span className="text-lg">
                      {platformIcons[v.platform as Platform]}
                    </span>
            ...
            <div className="sticky bottom-0 rounded-xl border bg-card p-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <span className="text-sm">
                  已选 <strong>{selectedPlatforms.size}</strong> 个平台
                </span>
