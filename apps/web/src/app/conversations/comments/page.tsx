...
const columns: ColumnDef<Interaction>[] = [
  ...
  { accessorKey: 'content', header: '评论内容', cell: ({ getValue }) => <span className="text-sm line-clamp-1">{getValue() as string}</span> },
...
export default function CommentsPage() {
  ...
  const syncMutation = useMutation({
    ...
    onSuccess: (result) => {
      setSyncMsg({ type: 'success', text: `评论同步已触发（任务 ${result.syncJobId.slice(0, 8)}…），后台处理中` });
      setTimeout(() => {
