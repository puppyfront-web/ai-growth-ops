...
const typeLabels: Record<string, string> = { comment: '评论', message: '私信', official_message: '公号消息', form_submission: '表单' };

const columns: ColumnDef<Interaction>[] = [
  { accessorKey: 'externalUserName', header: '用户', cell: ({ row }) => (
    <Link href={`/conversations/${row.original.conversationId ?? row.original.id}`} className="font-medium text-sm hover:underline">{row.original.externalUserName}</Link>
  )},
  { accessorKey: 'platform', header: '平台', cell: ({ getValue }) => <PlatformBadge platform={getValue() as string} /> },
  { accessorKey: 'type', header: '类型', cell: ({ getValue }) => <span className="text-sm">{typeLabels[getValue() as string] ?? getValue() as string}</span> },
  { accessorKey: 'content', header: '内容摘要', cell: ({ getValue }) => <span className="text-sm text-muted-foreground line-clamp-1 max-w-xs">{getValue() as string}</span> },
  { accessorKey: 'riskLevel', header: '风险', cell: ({ getValue }) => { const v = getValue() as string | null; return v ? <RiskBadge level={v} /> : <span className="text-xs text-muted-foreground">-</span>; } },
...
export default function ConversationsPage() {
  ...
  const { data, isLoading, error, refetch } = useQuery({ queryKey: ['interactions', filters], queryFn: () => listInteractions(filters) });
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-4">
        <select value={filters.platform ?? ''} onChange={(e) => setFilters((f) => ({ ...f, platform: e.target.value }))} className="rounded-md border p-2 text-sm">
      ...
      <DataTable columns={columns} data={data ?? []} loading={isLoading} error={error?.message} onRetry={() => refetch()} emptyTitle="暂无消息" emptyDescription="新评论和私信将在这里显示" />
    </div>
  );
}
