  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 text-muted-foreground text-xs">
        ...
        <span className="ml-2">ID: {c.id ? String(c.id).slice(0, 8) : 'N/A'}…</span>
      </div>
    );
  }
});
  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 text-xs">
        <span
          className={
            passed ? 'text-green-600 font-medium' : 'text-red-600 font-medium'
          }
        >
        ...
        {!passed && issues.length > 0 && (
          <ul className="mt-1 list-inside list-disc text-muted-foreground">
            {issues.slice(0, 5).map((iss, i) => (
              <li key={i}>
                {String(iss.message || iss.type || JSON.stringify(iss))}
              </li>
            ))}
    ...
    return (
      <div className="mt-1 flex flex-wrap gap-1 text-xs">
        {(result as Array<Record<string, unknown>>).map((v, i) => (
          <span key={i} className="rounded bg-accent px-1.5 py-0.5">
            {String(v.platform || `变体${i + 1}`)}
          </span>
        ))}
      </div>
    );
      ...
      <div className="mt-1 space-y-0.5 text-xs">
        {accounts.slice(0, 8).map((a) => (
          ...
          >
            ...
            <span
              className={
                a.hasCookie !== false ? 'text-green-600' : 'text-orange-500'
              }
            >
      ...
      <div className="mt-1 text-xs">
        ...
        {(r.platformAccount as Record<string, unknown>)?.platform ? (
          <span className="ml-2 text-muted-foreground">
            {String((r.platformAccount as Record<string, unknown>).platform)}
          </span>
        ) : null}
      </div>
    );
...
const ListInteractionsTool = makeAssistantToolUI({
  ...
  render: ({ result }) => {
    ...
    const r = result as Record<string, unknown>;
    const items = (r.items || (Array.isArray(result) ? result : [])) as Array<
...
const ClassifyInteractionTool = makeAssistantToolUI({
  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 text-xs">
        <span className={levelColors[String(c.leadLevel)] || ''}>
          等级 {String(c.leadLevel)}
        </span>
        <span className="ml-2">{String(c.intent)}</span>
        <span className="ml-2 text-muted-foreground">
          置信度 {Math.round((Number(c.confidence) || 0) * 100)}%
        </span>
      </div>
    );
  }
  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 grid grid-cols-2 gap-1 text-muted-foreground text-xs">
        {r.totalInteractions != null && (
          <div>互动: {String(r.totalInteractions)}</div>
        )}
        {r.totalLeads != null && <div>线索: {String(r.totalLeads)}</div>}
        {r.totalContentItems != null && (
          <div>内容: {String(r.totalContentItems)}</div>
        )}
        {r.totalPublishJobs != null && (
          <div>发布: {String(r.totalPublishJobs)}</div>
        )}
      </div>
    );
  }
  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 space-y-1 text-muted-foreground text-xs">
        ...
        <div>
          自动回复率: {Math.round((Number(r.autoReplyRate) || 0) * 100)}%
        </div>
        <div>
          线索转化率: {Math.round((Number(r.leadConversionRate) || 0) * 100)}%
        </div>
        {r.leadsByLevel ? (
          <div>
            线索等级:{' '}
            {Object.entries(r.leadsByLevel as Record<string, unknown>)
              .map(([k, v]) => `${k}: ${String(v)}`)
              .join(', ')}
          </div>
        ) : null}
      ...
      <div className="mt-1 space-y-0.5 text-xs">
        {items.slice(0, 5).map((c, i) => (
          <div key={i} className="text-muted-foreground">
            ...
            <span className="ml-2">
              互动:{String(c.interactionCount)} 线索:{String(c.leadCount)} 评分:
              {String(c.engagementScore)}
            </span>
          </div>
        ))}
  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 space-y-1 text-xs">
        ...
        {r.platforms ? (
          <div>平台: {(r.platforms as string[]).join(', ')}</div>
        ) : null}
        <div>
          已发布: {String(r.publishedCount)}{' '}
          {r.maxPostsTotal ? `/ ${String(r.maxPostsTotal)}` : ''}
        </div>
        {(r.runs as Array<Record<string, unknown>> | undefined)
          ?.slice(0, 3)
          .map((run, i) => (
            <div key={i} className="text-muted-foreground text-xs">
              {run.status === 'completed'
                ...
                : run.status === 'failed'
                  ? '❌'
                  : '⏳'}{' '}
              {String(run.status)} {String(run.createdAt)?.slice(0, 10)}
            </div>
          ))}
      </div>
    );
  ...
  render: ({ result }) => {
    ...
    return (
      <div className="mt-1 space-y-1 text-xs">
        ...
        {steps.length > 0 && (
          <div className="text-muted-foreground">步骤: {steps.length} 个</div>
        )}
          ...
          <div key={i} className="text-muted-foreground text-xs">
            {exec.status === 'completed'
              ...
              : exec.status === 'failed'
                ? '❌'
                : '⏳'}{' '}
            {String(exec.status)} 步骤 {String(exec.currentStepIndex || 0)}
          </div>
        ))}
      </div>
    );
    ...
    return (
      <div className="mt-1 space-y-1 text-xs">
        ...
        {suggestions.length === 0 && (
          <div className="text-green-600">✅ 一切正常，暂无待办事项</div>
        )}
      </div>
    );
  }
        ...
        {templates.map((t, i) => (
          <div key={i} className="text-muted-foreground">
            ...
            <span className="ml-2 opacity-60">
              ({String(t.stepCount)}步) {String(t.description)}
            </span>
          </div>
        ))}
      </div>
    ...
    return (
      <div className="mt-1 text-xs">
        ...
        <div className="text-muted-foreground">
          共 {String(r.totalSteps)} 步
        </div>
        <div className="mt-1 space-y-0.5">
          {steps.map((s, i) => (
            <div key={i} className="text-muted-foreground">
              {i + 1}. {String(s.description)}{' '}
              <span className="opacity-60">({String(s.toolName)})</span>
            </div>
          ))}
        </div>
      </div>
...
const SearchVideoCommentsTool = makeAssistantToolUI({
  ...
  render: ({ result }) => {
    ...
    const r = result as Record<string, unknown>;
    const prospects = (r.prospects || []) as Array<{
      userName?: string;
      content?: string;
      leadLevel?: string;
      intent?: string;
          ...
          <div className="space-y-1">
            ...
            {prospects.slice(0, 10).map((p, i) => (
              <div key={i} className="bg-accent/50 rounded px-2 py-1">
                ...
                {p.videoTitle ? (
                  <div className="text-muted-foreground opacity-60 truncate">
                    来自: {String(p.videoTitle)}
                  </div>
                ) : null}
              </div>
            ))}
...
function EntityCreated({ result }: { result: unknown }) {
  ...
  return (
    <div className="mt-1 text-xs">
      <span className="font-medium text-foreground">{String(r.name)}</span>
      <span className="ml-2 text-muted-foreground">
        ID: {r.id ? String(r.id).slice(0, 8) : 'N/A'}…
      </span>
...
function AutoReplyConfig({ result }: { result: unknown }) {
  ...
  return (
    <div className="mt-1 space-y-0.5 text-muted-foreground text-xs">
      ...
      <div>
        静默期: {String(r.quietHoursStart)} - {String(r.quietHoursEnd)}
      </div>
    </div>
  );
}
...
function ExecutionTriggered({ result }: { result: unknown }) {
  ...
  return (
    <div className="mt-1 text-green-600 font-medium text-xs">
      ✅ 已触发执行{' '}
      {r.executionId
        ? `(ID: ${String(r.executionId).slice(0, 8)}…)`
        : r.runId
...
function PreferencesDisplay({ result }: { result: unknown }) {
  ...
  if ((r.preferredPlatforms as string[])?.length)
    prefLines.push(`平台: ${(r.preferredPlatforms as string[]).join(', ')}`);
  if (r.contentStylePreferences)
    prefLines.push(`内容风格: ${String(r.contentStylePreferences)}`);
  if (r.replyStylePreferences)
    prefLines.push(`回复风格: ${String(r.replyStylePreferences)}`);
  if (r.brandVoice) prefLines.push(`品牌调性: ${String(r.brandVoice)}`);
  if ((r.preferredPublishTimes as string[])?.length)
    prefLines.push(
      `发布时间: ${(r.preferredPublishTimes as string[]).join(', ')}`
    );
  return (
    <div className="mt-1 space-y-0.5 text-muted-foreground text-xs">
      {prefLines.length > 0 ? (
        prefLines.map((l, i) => <div key={i}>{l}</div>)
      ) : (
        <div>暂无偏好设置</div>
      )}
    </div>
