...
import { Send, Eye, Users, Lightbulb } from 'lucide-react';
...
function computeTrend(data: TrendDataPoint[] | undefined): number | undefined {
  ...
  const avgFirst =
    firstHalf.reduce((s, d) => s + (Number(d.total) || 0), 0) /
    firstHalf.length;
  const avgSecond =
    secondHalf.reduce((s, d) => s + (Number(d.total) || 0), 0) /
    secondHalf.length;
