...
export const createPlatformAccountSchema = z.object({
  platform: z.enum(['douyin', 'xiaohongshu', 'wechat_official', 'wechat_channels', 'baijiahao', 'zhihu']),
  name: z.string().min(1, '请输入账号名称'),
  mode: z.enum(['official_api', 'browser_assist', 'manual_confirm', 'manual_import']).default('official_api'),
...
export const updatePlatformAccountSchema = z.object({
  ...
  mode: z.enum(['official_api', 'browser_assist', 'manual_confirm', 'manual_import']).optional(),
  capabilities: z.record(z.unknown()).optional(),
});
...
export const createOrgSchema = z.object({
  name: z.string().min(1, '请输入组织名称').max(100, '组织名称不能超过100个字符'),
});
