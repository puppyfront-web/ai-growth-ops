...
export async function executeResearchTaskSync(
  ...
  if (!['DRAFT', 'PAUSED', 'FAILED'].includes(task.status)) {
    throw new ResearchExecutionError(
      400,
      'INVALID_STATE',
      `Cannot run from ${task.status} state`
    );
  }
  ...
  try {
    ...
    const platform = Array.isArray(task.platforms)
      ? String(task.platforms[0] ?? 'douyin')
      : 'douyin';
    const keywords = Array.isArray(task.keywords)
      ? task.keywords.map((item) => String(item))
      : [];
    ...
    for (const keyword of keywords.length > 0 ? keywords : ['热门内容']) {
      try {
        ...
        if (!resp.ok) {
          ...
          console.error(
            `[research] Search failed for "${keyword}": ${resp.status} ${errText}`
          );
          continue;
        }
        ...
        for (const video of results) {
          allPosts.push({
            ...
            commentCount:
              (video.comments as Record<string, unknown>[])?.length ?? 0,
            metadata: { keyword }
          });
        ...
        console.log(
          `[research] Keyword "${keyword}": ${results.length} videos, ${allComments.length} total comments`
        );
      } catch (err) {
        console.error(
          `[research] Search error for "${keyword}":`,
    ...
    console.log(
      `[research] Stored ${allPosts.length} posts, ${allComments.length} comments`
    );
    ...
    console.log(
      `[research] Task ${researchTaskId} completed: INSIGHT_GENERATED`
    );
    ...
    console.error(
      `[research] Task ${researchTaskId} failed:`,
    ...
    await db.researchTask
      ...
      .catch(() => {
        /* update may fail if record gone */
      });
    throw error;
  }
}
...
async function clearPreviousOutputs(
  db: DatabaseClient,
  researchTaskId: string
): Promise<void> {
    ...
    const result = await runner.run({
      ...
      input: {
        ...
        comments: input.comments
          .slice(0, 50)
          .map((c) => ({ text: c.content, user: c.externalUserName })),
    ...
    if (result.status === 'success' && result.output) {
      ...
      const insightDefs = [
        { type: 'pain_points', title: '用户痛点分析', data: output.painPoints },
        {
          type: 'popular_topics',
          title: '热门话题趋势',
          data: output.popularTopics
        },
        {
          type: 'content_angles',
          title: '内容创作角度',
          data: output.contentAngles
        }
      ];
      ...
      console.log(
        `[research] AI insights generated: ${insightDefs.length} insights, ${opportunities.length} opportunities`
      );
      return;
    }
  } catch (err) {
    console.error(
      '[research] AI insight generation failed, falling back to rule-based:',
      err instanceof Error ? err.message : String(err)
    );
  }
