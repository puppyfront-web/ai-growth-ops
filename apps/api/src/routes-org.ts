...
export const orgRoutes: Array<{
  method: string;
  pattern: string;
  handler: (req: IncomingMessage, res: ServerResponse, ctx: OrgRouteContext) => Promise<void>;
}> = [
  // ── GET /api/org — List user's organizations ─────────────────────────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      const orgs = memberships.map(m => ({
        ...
        role: m.role,
        isDefault: (m.organization.metadata as Record<string, unknown>)?.isDefault === true,
      }));
  ...
  // ── GET /api/org/:orgId — Get organization detail ────────────────────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      const membership = await ctx.db.organizationMember.findFirst({
        where: { organizationId: ctx.params.orgId, userId: user.id, status: 'active' },
        include: { organization: true },
      });
      if (!membership) return sendJson(res, 404, { error: '组织不存在或无权访问' });
  ...
  // ── PATCH /api/org/:orgId — Update organization ──────────────────────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      if (!data.name && data.name !== undefined) return sendJson(res, 400, { error: '组织名称不能为空' });
  ...
  // ── POST /api/org/:orgId/invite — Invite member ──────────────────────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      if (!['member', 'admin', 'viewer'].includes(role)) return sendJson(res, 400, { error: '无效的角色' });
      ...
      if (existingUser) {
        ...
        if (existingMember) return sendJson(res, 409, { error: '该用户已是组织成员' });
      }
  ...
  // ── POST /api/org/:orgId/accept-invite — Accept invitation ───────────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      const invitation = await ctx.db.organizationInvitation.findUnique({ where: { token } });
      if (!invitation) return sendJson(res, 400, { error: '邀请令牌无效' });
      if (invitation.organizationId !== ctx.params.orgId) return sendJson(res, 400, { error: '邀请与组织不匹配' });
      if (invitation.status !== 'pending') return sendJson(res, 400, { error: '邀请已处理' });
      if (invitation.expiresAt < new Date()) return sendJson(res, 400, { error: '邀请已过期' });
  ...
  // ── DELETE /api/org/:orgId/members/:memberId — Remove member ─────────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      if (member.role === 'owner') return sendJson(res, 400, { error: '不能移除组织所有者' });

      await ctx.db.organizationMember.delete({ where: { id: ctx.params.memberId } });
  ...
  // ── PATCH /api/org/:orgId/members/:memberId/role — Change role ───────────
  {
    ...
    handler: async (req, res, ctx) => {
      ...
      if (!['admin', 'member', 'viewer'].includes(role)) return sendJson(res, 400, { error: '无效角色' });
      ...
      if (member.role === 'owner') return sendJson(res, 400, { error: '不能变更所有者角色' });
