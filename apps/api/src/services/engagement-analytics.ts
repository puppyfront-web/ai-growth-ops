...
export async function aggregateEngagementMetrics(
  ...
  const [interactions, classifications, leads, replySuggestions] = await Promise.all([
    // Total interactions in period
    db.interaction.findMany({
      where: { organizationId: orgId, receivedAt: { gte: since }, deletedAt: null },
  ...
  for (const i of interactions) {
    interactionsByPlatform[i.platform] = (interactionsByPlatform[i.platform] || 0) + 1;
    interactionsByType[i.type] = (interactionsByType[i.type] || 0) + 1;
  }

  // Reply rate
  const repliedCount = interactions.filter(i => i.status === 'REPLIED').length;
  const replyRate = interactions.length > 0 ? repliedCount / interactions.length : 0;

  // Auto-reply rate
  const sentReplies = replySuggestions.filter(r => r.status === 'sent').length;
  const autoReplyRate = replySuggestions.length > 0 ? sentReplies / replySuggestions.length : 0;

  // Average confidence
  const confidences = classifications.map(c => c.confidence).filter(Boolean);
  const avgClassificationConfidence = confidences.length > 0
  ...
  const leadConversionRate = interactions.length > 0 ? leads.length / interactions.length : 0;
...
export async function computeContentPerformance(
  ...
  for (const job of publishJobs) {
    ...
    existing.replyCount += job.interactions.filter(i => i.status === 'REPLIED').length;

    // Engagement score: weighted combination
    existing.engagementScore = existing.interactionCount + existing.replyCount * 2 + existing.leadCount * 5;

    byContent.set(ci.id, existing);
  }

  return Array.from(byContent.values()).sort((a, b) => b.engagementScore - a.engagementScore);
}
