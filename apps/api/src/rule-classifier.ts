 ...
 * Shared between API routes and worker pipeline.
 *
...
export function classifyByRules(content: string): ClassificationResult {
  ...
  if (text.match(/批量|采购|批发|供货|经销商|代理|加盟|商务合作|合作|公司想|企业/)) {
    return {
  ...
  // Price inquiry
  if (text.match(/多少钱|价格|费用|报价|收费标准|怎么卖|在哪买|哪里买|购买|cost|price/)) {
    return {
  ...
  if (text.match(/投诉|差评|不满|失望|垃圾|骗|退款|假货|难用|坑|别买|退货.*不|complaint|terrible/)) {
    return {
      intent: 'complaint',
      leadLevel: 'C',
      confidence: 0.85,
  ...
  if (text.match(/质量|怎么样|能用|耐用|售后|保修|换货|退货|材质|规格|参数|介绍|详细/)) {
    return {
...
export function generateRuleBasedReply(
  ...
  if (intent === 'complaint' || text.match(/投诉|差评|不满|失望|垃圾|骗|退款|假货|难用|坑|别买/)) {
    return {
      suggestedText: '非常抱歉给您带来不好的体验，我们非常重视您的反馈。请私信我们详细情况，我们会尽快核实并为您妥善处理。',
  ...
  if (intent === 'cooperation_inquiry' || text.match(/合作|代理|加盟|批发|批量|采购/)) {
    return {
      suggestedText: '您好，感谢您对我们产品的关注！关于合作事宜，可以私信我详细沟通，我会安排商务同事尽快与您对接，为您提供最合适的合作方案。',
  ...
  if (intent === 'price_inquiry' || text.match(/多少钱|价格|怎么卖|在哪买|哪里买|购买/)) {
    return {
      suggestedText: '您好，具体价格会根据您选择的规格和数量有所不同。可以私信我了解详细方案和报价，这边安排同事为您详细介绍哦～',
      replyType: 'guide_to_private',
      riskLevel: 'low',
      needReview: false,
    };
  ...
  if (intent === 'appointment_request' || text.match(/预约|演示|体验/)) {
    return {
      suggestedText: '您好，感谢您的关注！可以私信我您的联系方式，这边安排专业顾问为您预约演示体验。',
  ...
  if (intent === 'product_inquiry' || text.match(/质量|怎么样|能用|售后|介绍|详细|产品|功能/)) {
    return {
      suggestedText: '您好，我们的产品品质有保障，提供完善的售后服务。可以私信我了解详细的产品参数和使用案例，这边为您详细介绍！',
  ...
  if (intent === 'positive_engagement' || text.match(/关注|不错|喜欢|支持/)) {
    return {
      suggestedText: '感谢关注和支持！我们会持续分享优质内容，有需要随时私信我们哦～',
      replyType: 'thanks',
      riskLevel: 'low',
      needReview: false,
    };
  ...
  return {
    suggestedText: '感谢您的关注！如有任何问题欢迎随时私信我们，我们会尽快为您解答。',
    replyType: 'thanks',
    riskLevel: 'low',
    needReview: false,
  };
