...
import type { DefaultSkillRunner, SkillRunResult } from '@ai-growth-ops/skills';
...
export async function generateInsights(input: GenerateInsightsInput): Promise<InsightResult> {
...
async function generateAIInsights(input: GenerateInsightsInput): Promise<InsightResult> {
  ...
  const questionComments = input.comments.filter(c =>
    c.content.includes('?') || c.content.includes('？') ||
    c.content.includes('怎么') || c.content.includes('如何') ||
    c.content.includes('多少') || c.content.includes('请问')
  );
  if (questionComments.length > 0) {
    insights.opportunities.push({
      title: 'FAQ 内容选题',
      description: `发现${questionComments.length}条提问类评论，可以整理为FAQ内容选题。常见问题：${questionComments.slice(0, 3).map(c => c.content.slice(0, 20)).join('、')}`,
      platforms: [input.platform],
      priority: 'medium',
    });
  }
...
function analyzeCommentSentiment(comments: Array<{ content: string }>): {
  ...
  const questionWords = ['怎么', '如何', '多少', '请问', '为什么', '吗', '？', '?'];
