...
export async function discoverSkillManifests(dir: string): Promise<SkillManifestRecord[]> {
  const entries = await readdir(dir, { withFileTypes: true });
  const files = entries.filter((entry) => entry.isFile() && entry.name.endsWith('.json'));
  const manifests = await Promise.all(files.map((file) => loadSkillManifest(join(dir, file.name))));
  return manifests;
}
