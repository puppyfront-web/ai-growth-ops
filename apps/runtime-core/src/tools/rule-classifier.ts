...
export function generateRuleBasedReply(
  ...
  if (intent === 'price_inquiry' || text.match(/多少钱|价格/)) {
    return {
      suggestedText: '您好，具体价格会根据您的需求有所不同。可以私信我了解详细方案和报价，我这边安排同事为您详细介绍。',
  ...
  if (intent === 'appointment_request' || text.match(/预约/)) {
    return {
      suggestedText: '您好，感谢您的关注！可以私信我您的联系方式，我这边安排专业顾问为您预约演示。',
  ...
  if (intent === 'cooperation_inquiry' || text.match(/合作|代理|加盟/)) {
    return {
      suggestedText: '您好，感谢您对我们产品的关注！关于合作事宜，可以私信我详细沟通，我会安排商务同事与您对接。',
      replyType: 'guide_to_private',
      riskLevel: 'medium',
      needReview: true,
  ...
  if (intent === 'product_inquiry' || text.match(/产品|方案|功能/)) {
    return {
      suggestedText: '您好，我们提供完整的解决方案。可以私信我了解详细方案和案例参考。',
  ...
  if (intent === 'complaint' || text.match(/投诉|差评|不满/)) {
    return {
      suggestedText: '非常抱歉给您带来不好的体验。我们会尽快核实并处理您的问题，请私信我们详细情况。',
