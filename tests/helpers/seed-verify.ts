...
export async function verifySeedData(db: DatabaseClient): Promise<void> {
  ...
  if (accounts !== 6)
    throw new Error(`Expected 6 platform accounts, got ${accounts}`);

  const content = await db.contentItem.count();
  if (content !== 7)
    throw new Error(`Expected 7 content items, got ${content}`);

  const variants = await db.contentVariant.count();
  if (variants < 6)
    throw new Error(`Expected >= 6 content variants, got ${variants}`);
  ...
  if (interactions !== 6)
    throw new Error(`Expected 6 interactions, got ${interactions}`);
...
export async function getSeedIds(db: DatabaseClient) {
  const user = await db.user.findFirstOrThrow({
    where: { email: 'customer-demo@ai-growth-ops.local' }
  });
  const accounts = await db.platformAccount.findMany({
    where: { userId: user.id }
  });
  const project = await db.contentProject.findFirstOrThrow({
    where: { userId: user.id }
  });
