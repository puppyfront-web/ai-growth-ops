...
describe('runLeadMining pipeline', () => {
  it('classifies candidates and returns structured result', async () => {
    ...
    mockRunClassification
      .mockResolvedValueOnce({
        result: { intent: 'price_inquiry', leadLevel: 'A', confidence: 0.88, riskLevel: 'low', summary: '用户咨询价格', tags: ['价格咨询'], nextAction: 'suggest_reply' },
        source: 'rules',
      })
      .mockResolvedValueOnce({
        result: { intent: 'general_chat', leadLevel: 'C', confidence: 0.5, riskLevel: 'low', summary: '一般互动', tags: [], nextAction: 'suggest_reply' },
        source: 'rules',
      })
      .mockResolvedValueOnce({
        result: { intent: 'spam', leadLevel: 'D', confidence: 0.9, riskLevel: 'medium', summary: '疑似推广', tags: ['垃圾'], nextAction: 'ignore' },
        source: 'rules',
      });

    mockRunReplySuggestion
      .mockResolvedValueOnce({
        result: { suggestedText: '您好，具体价格...', replyType: 'guide_to_private', riskLevel: 'low', needReview: false },
        source: 'rules',
      })
      .mockResolvedValueOnce({
        result: { suggestedText: '感谢关注', replyType: 'thanks', riskLevel: 'low', needReview: false },
        source: 'rules',
      });
  ...
  it('D-level leads get no reply suggestion', async () => {
    mockRunClassification.mockResolvedValueOnce({
      result: { intent: 'spam', leadLevel: 'D', confidence: 0.9, riskLevel: 'medium', summary: 'spam', tags: [], nextAction: 'ignore' },
      source: 'rules',
    });
  ...
  it('non-D leads get reply suggestions', async () => {
    mockRunClassification.mockResolvedValueOnce({
      result: { intent: 'price_inquiry', leadLevel: 'A', confidence: 0.9, riskLevel: 'low', summary: 'Price', tags: [], nextAction: 'suggest_reply' },
      source: 'llm',
    });
    mockRunReplySuggestion.mockResolvedValueOnce({
      result: { suggestedText: '您好，感谢咨询', replyType: 'guide_to_private', riskLevel: 'low', needReview: false },
      source: 'llm',
    });
    ...
    expect(result.leads[0].replySuggestion).toBeDefined();
    expect(result.leads[0].replySuggestion!.suggestedText).toBe('您好，感谢咨询');
  ...
  it('computes correct level breakdown', async () => {
    ...
    for (const level of levels) {
      mockRunClassification.mockResolvedValueOnce({
        result: { intent: 'test', leadLevel: level, confidence: 0.5, riskLevel: 'low', summary: '', tags: [], nextAction: 'suggest_reply' },
        source: 'rules',
      });
    }
    // Non-D leads get reply
    mockRunReplySuggestion.mockResolvedValue({
      result: { suggestedText: 'ok', replyType: 'thanks', riskLevel: 'low', needReview: false },
      source: 'rules',
    });
  ...
  it('preserves original content and metadata', async () => {
    mockRunClassification.mockResolvedValueOnce({
      result: { intent: 'test', leadLevel: 'A', confidence: 0.9, riskLevel: 'low', summary: 'test', tags: [], nextAction: 'suggest_reply' },
      source: 'rules',
    });
    mockRunReplySuggestion.mockResolvedValueOnce(null);

    const result = await runLeadMining({
      candidates: [makeCandidate({ content: '合作事宜', platform: 'douyin', interactionType: 'message', userNickname: 'test_user' })],
    });
