...
class SandboxPublishProvider implements PublishProvider {
  ...
  async publishTextImage(_input: {
    ...
    if (!this.caps.textImage) {
      return {
        ...
        errorMessage: `${this.platform} does not support text_image publishing`
      };
    }
    return {
      success: true,
      externalPublishId: `sandbox_${this.platform}_${Date.now()}`
    };
  }

  async publishVideo(_input: {
    title: string;
    videoAssetId: string;
  }): Promise<PublishResult> {
    if (!this.caps.video) {
      return {
        success: false,
        errorCode: 'UNSUPPORTED',
        errorMessage: `${this.platform} does not support video publishing`
      };
    }
    return {
      success: true,
      externalPublishId: `sandbox_${this.platform}_vid_${Date.now()}`
    };
  }
  ...
  async fetchStatus(input: {
    ...
    return {
      status: 'published',
      externalUrl: `https://${this.platform}.com/post/${input.externalPublishId}`
    };
  }
}
...
describe('PublishProvider Contract', () => {
  ...
  it('unsupported capability returns clear error', async () => {
    const limited = new SandboxPublishProvider('wechat_official', {
      textImage: true,
      video: false,
      article: false,
      answer: false
    });
    const result = await limited.publishVideo({
      title: 'Test',
      videoAssetId: 'a1'
    });
